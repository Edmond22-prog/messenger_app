...
var socket = io.connect("http://10.0.2.2:300");
...
socket.on("notWritting", (pseudo) => {
  console.log("is not writing ", pseudo);
  document.getElementById("isWritting").textContent = "";
});
...
socket.on("emitChannel", (channel) => {
  if (channel.previousChannel) {
    document
      .getElementById(channel.previousChannel)
      .classList.remove("inChannel");
  }
  ...
  switch (element) {
    ...
    case "whisper":
      ...
      newElement.textContent =
        content.sender + " vous chuchote: " + content.message;
      document.getElementById("msgContainer").appendChild(newElement);
      break;
    ...
    case "oldWhispers":
      newElement.classList.add(element, "message");
      newElement.textContent =
        content.sender + " vous chuchote: " + content.content;
      document.getElementById("msgContainer").appendChild(newElement);
      break;
  }
}
...
function createChannel(newRoom) {
  ...
  document
    ...
    .insertBefore(newRoomItem, document.getElementById("createNewRoom"));
}
